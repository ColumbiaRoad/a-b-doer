...
if(!browser){console.log((0,_chalk.green)("Starting browser..."));console.log();browser=await _puppeteerCore.default.launch({headless:Boolean(config.headless),devtools:Boolean(config.devtools),executablePath:config.browser,userDataDir:config.userDataDir||undefined,defaultViewport:null,args:[`--window-size=${(config.windowSize||windowSize)[0]},${(config.windowSize||windowSize)[1]}`,"--incognito"]});aboutpage=(await browser.pages())[0];aboutpage.evaluate(()=>document.body.innerHTML=`<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="margin: auto; background: rgb(255, 255, 255); display: block;" width="200px" height="200px" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid">
