	...
	if (!watch) {
		...
		if (!TEST_ENV) {
			...
			fs.readFile(path.resolve(buildDir, bundleName), (err, data) => {
				...
				const bundleSize = toKb(bundleStr);

				const jsSizeLine =
					(!stylesOnly && mainChunk.fileName.padEnd(len) + toKb(mainChunk.code).padStart(bundleSize.length)) || '';
				const cssSizeLine =
					(cssChunk && cssChunk.fileName.padEnd(len) + toKb(cssChunk.source).padStart(bundleSize.length)) || '';

				const bundleSizeLine = bundleName.padEnd(len) + bundleSize;
