...
export async function bundler(testConfig) {
	...
	const createAssetBundle = () => {
		...
		try {
			...
			if (stylesOnly) {
				fs.unlinkSync(path.resolve(buildDir, mainChunk.fileName));
			}
			// Include the js content
			else {
				js = fs.readFileSync(path.resolve(buildDir, mainChunk.fileName), 'utf8');
				...
				if (fnIifeCheck.test(js) && outputOptions.format === 'iife') {
					js = '!' + js;
					fs.writeFileSync(path.resolve(buildDir, mainChunk.fileName), js);
				}
			}
			styles = fs.readFileSync(path.resolve(buildDir, cssChunk.fileName), 'utf8');
		} catch (error) {}

		let assetBundle = js;

		const sourceMapMatch = js.match(/\/\/#\s*sourceMappingURL=.+/s);
		let jsWithoutSourceMap = js;
		if (sourceMapMatch) {
			jsWithoutSourceMap = jsWithoutSourceMap.replace(sourceMapMatch, '');
			sourcemap = sourceMapMatch[0];
		}
		...
		fs.writeFileSync(path.resolve(buildDir, getFileAs(mainChunk.fileName, 'bundle.js')), assetBundle);
