...
export async function openPage(config, singlePage) {
	...
	if (!isTest) {
		console.log(
			...[`#${++counter}`, initial ? 'Opening' : 'Reloading', `page ${yellow(url)}`].concat(
				pageTags.length ? ['with custom:', pageTags] : []
			)
		);
	}
...
export async function getBrowser(config = {}) {
	...
	if (!browser) {
		...
		browser = await puppeteerCore.launch({
			...
			devtools: Boolean(config.devtools ?? true),
			executablePath: config.browser,
			userDataDir: config.userDataDir || undefined,
			defaultViewport: null,
			ignoreDefaultArgs: ['--disable-extensions'],
			args: [
				`--window-size=${(config.windowSize || windowSize)[0]},${(config.windowSize || windowSize)[1]}`,
				'--incognito',
			],
		});
