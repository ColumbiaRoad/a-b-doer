...
options.vnode = (vnode) => {
	if (vnode && typeof vnode.type === 'function') {
		...
		if (vnode.__class && 'prototype' in vnode.type && vnode.type.prototype.render) {
			vnode.__class.constructor = vnode.type;
		}
	}
	oldVnode(vnode);
};
...
function replaceComponent(OldType, NewType, resetHookState) {
	...
	vnodes.forEach((vnode, index) => {
		...
		if (vnode.__class) {
			...
			if (vnode.__class instanceof OldType) {
				const oldInst = vnode.__class;
				const newInst = new NewType(vnode.props);
				vnode.__class = newInst;
		...
		patchVnodeDom(newVnode, oldVnode, null, 'same');
